	...
	int mode) 		// Режим отображения окна
{
	...
	wc.hInstance = This;
	wc.lpszClassName = Win1; 				// Имя класса окна 
	...
	// Создание окна 
	hWnd = CreateWindow(Win1,			// Имя класса окна 
		_T("Window1"), 		// Заголовок окна 
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_LBUTTONDOWN:
		hWnd2 = FindWindow(Win2, NULL);
		if (hWnd2 == NULL)
			MessageBox(hWnd, _T("Окно2 не запущено."), _T("Win1"), MB_OK);
		else
		{
			MessageBox(hWnd, _T("Окно2 найдено."), _T("Win1"), MB_OK);
			MessageBox(hWnd, _T("Дискриптор окна1 передано."), _T("Win1"), MB_OK);
